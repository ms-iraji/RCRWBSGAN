...
class DenseNet201(nn.Module):
    ...
    def __init__(self, out_size, mode, drop_rate=0):
        ...
        assert mode in ('U-Ones', 'U-Zeros', 'U-MultiClass')
        self.densenet201                   = densenet.densenet201                  (pretrained=True, drop_rate=drop_rate)
        ...
        if mode in ('U-Ones', 'U-Zeros'):
            ...
            self.densenet201                  .classifier = None
            self.densenet201                  .Linear_0 = nn.Linear(num_ftrs, out_size)
            self.densenet201                  .Linear_1 = nn.Linear(num_ftrs, out_size)
            self.densenet201                  .Linear_u = nn.Linear(num_ftrs, out_size)
            
        self.mode = mode
        
        # Official init from torch repo.
        for m in self.densenet201                  .modules():
            if isinstance(m, nn.Linear):
                nn.init.constant_(m.bias, 0)
    ...
    def forward(self, x):
        features = self.densenet201                  .features(x)
        out = F.relu(features, inplace=True)
        ...
        self.activations = out
        if self.mode in ('U-Ones', 'U-Zeros'):
            out = self.densenet201                  .classifier(out)
        elif self.mode in ('U-MultiClass', ):
            n_batch = x.size(0)
            out_0 = self.densenet201                  .Linear_0(out).view(n_batch, 1, -1)
            out_1 = self.densenet201                  .Linear_1(out).view(n_batch, 1, -1)
            out_u = self.densenet201                  .Linear_u(out).view(n_batch, 1, -1)
            out = torch.cat((out_0, out_1, out_u), dim=1)
